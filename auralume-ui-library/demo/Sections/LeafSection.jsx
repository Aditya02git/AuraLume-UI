...
import Leaf from "../../src/components/ThreeJs/Leaf";
...
const ThreeCanvas = ({ modelPath, effectProps, canvasId }) => {
  ...
  useEffect(() => {
    ...
    const directionalLight = new THREE.DirectionalLight(0xFFC300, 5);
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Setup animation mixer and play animations  ← ADD ALL THIS
        if (gltf.animations && gltf.animations.length > 0) {
        console.log('Animations found:', gltf.animations.map(a => a.name));
        mixerRef.current = new THREE.AnimationMixer(model);
        
        const windmillAnimation = gltf.animations.find(anim => anim.name === 'windmill');
        if (windmillAnimation) {
            const action = mixerRef.current.clipAction(windmillAnimation);
            action.setLoop(THREE.LoopRepeat, Infinity);
            action.play();
            console.log('Playing windmill animation in infinite loop');
        } else {
            console.log('Windmill animation not found, playing first animation');
...
const App = () => {
  ...
  const handleEffectGroupCreated = (group) => {
    if (sceneRef.current && group) {
      sceneRef.current.add(group);
      console.log('Wind effect added to scene:', group);
    }
  };
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', margin: 0, padding: 0 }}>
      ...
      {/* Leaves (rendered once scene ready) */}
      {sceneReady && (
        <Leaf
          ref={effectRef}
          position = {{ x: 0, y: 0, z: 0 }}
          onGroupCreated={handleEffectGroupCreated}
          key={\`leaves-effect-\${sceneReady}\`}
        />
      )}
    </div>
  );
...
`;
  ...
  const propsTableData = [
          { prop: "speed", type: "number", default: "1.6", desc: "Controls the overall animation speed of the leaf effect." },
          { prop: "animation", type: "boolean", default: "true", desc: "Enables or disables the leaf animation." },
          { prop: "numberOfLeafs", type: "number", default: "50", desc: "Total number of leaf particles generated." },
          { prop: "position", type: "object", default: "{ x: 0, y: 0, z: 0 }", desc: "Defines the 3D position of the leaf effect." },
          { prop: "rotation", type: "object", default: "{ x: 0, y: 0, z: 0 }", desc: "Specifies the rotation of the leaf system." },
          { prop: "mode", type: "string", default: "'flow'", desc: "Defines motion style — 'flow' for wind-blown leaves or 'whirl' for swirling leaves." },
          { prop: "area", type: "array<number>", default: "[10, 10, 5]", desc: "Defines the emission area for leaves in [width, depth, height]." },
          { prop: "leafSize", type: "number", default: "0.05", desc: "Base size of each individual leaf particle." },
          { prop: "leafSizeVariation", type: "number", default: "0.3", desc: "Controls random variation in leaf sizes (0–1 range)." },
          { prop: "fallSpeed", type: "number", default: "2", desc: "Determines how fast leaves fall (recommended 0.1–2)." },
          { prop: "swayAmount", type: "number", default: "20", desc: "Controls how much leaves sway or drift side-to-side while falling." },
          { prop: "rotationSpeed", type: "number", default: "10", desc: "Speed at which individual leaves rotate during descent." },
          { prop: "textureUrl", type: "string | null", default: "null", desc: "Optional custom texture URL for the leaves." },
          { prop: "color", type: "number", default: "0x7cb342", desc: "Default base color for leaves when no color array is provided." },
          { prop: "colors", type: "array<number>", default: "[0x7cb342, 0x8bc34a, 0x689f38, 0xffa726, 0xff7043]", desc: "Array of colors for natural variation between leaves." },
          { prop: "onGroupCreated", type: "function | null", default: "null", desc: "Callback function triggered when the Three.js group is created." },
          { prop: "ref", type: "React.Ref", default: "-", desc: "Forwarded ref for accessing the internal Three.js group." },
        ];
